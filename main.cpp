...
int main(int, char**)
{
	...
	if (patch_driver == 1)
	{
		...
		if (EnumDeviceDrivers(drivers_array, sizeof(drivers_array), &size_needed) && size_needed < sizeof(drivers_array))
		{
			...
			for (int i = 0; i < driver_count; i++)
			{
				...
				{
					data::registry::driver_address = drivers_array[i];
					printf("[+] *****.sys address: 0x%x\n", data::registry::driver_address);
		...
		if (wvpm(reinterpret_cast<uintptr_t>(data::registry::driver_address), sizeof(raw_driver) / sizeof(raw_driver[0]), (unsigned char*)&raw_driver))
			printf("[+] *****.sys was successfully patched!\n");
		else
		{
			printf("[-] *****.sys could not be patched!\n");
			system("pause");
			return 0;
	...
	if (find_kprocess(data::registry::cr3, "starwarsbattle", &(data::process::base_address), &(data::process::process_id)))
	{
		system("cls");
		printf("[+] SWBF2 found!\n");
